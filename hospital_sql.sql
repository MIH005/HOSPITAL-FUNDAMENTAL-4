...
-- Atualiza o status de dois médicos para inativos
-- Aqui vamos marcar os médicos com CRM 'CRM1001' e 'CRM1003' como inativos (FALSE)
UPDATE MEDICO
SET em_atividade = FALSE  -- Define o status como inativo
WHERE NUM_CRM IN ('CRM1001', 'CRM1003');  -- Especifica quais médicos atualizar

-- Atualiza todos os outros médicos para garantir que estão em atividade
UPDATE MEDICO
SET em_atividade = TRUE  -- Define o status como ativo
WHERE em_atividade IS NULL;  -- Aplica a mudança para médicos que não tinham status definido
